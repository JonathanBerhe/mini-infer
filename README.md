...
* **DeepSeek-V4 from-scratch contribution** — every published V4 primitive implemented and integrated through a runnable backbone:
   ...
   * **Backbone** (`DeepseekV4ForCausalLM`): per-layer CSA-or-HCA dispatch driven by `compress_ratios`. End-to-end prefill + decode runs with all primitives composed (CSA + HCA + Sink + Grouped Output + StateCache + hash-routed MoE + Hyper-Connections); 100+ V4-related tests cover the integration matrix. `load_weights` now walks the safetensors state_dict (FP8 e4m3fn auto-dequant to BF16, V2/V3-style MoE expert renames), then dispatches through `load_state_dict_with_tp` so each rank slices its share via the column/row/vocab-parallel layers. V4-Flash's NVFP4-packed expert weights raise a clear error pending the dequant kernel; the non-FP4 portion loads end-to-end. Validation against real V4-Flash on 2× B200 is the next milestone.
   * **Tensor parallelism**: Megatron-style column/row-parallel linears, vocab-parallel embedding, and expert-parallel MoE all live in `src/mini_infer/distributed/`. Every attention type (GQA, MLA, HCA, CSA), FFN type (SwiGLU, GeGLU, MoE, HashRoutedMoEFFN), and `embed_tokens` / `lm_head` are TP-aware. At `world_size=1` every module is bit-equivalent to its plain-PyTorch counterpart, so single-device behaviour is unchanged. Multi-process CPU parity tests (gloo backend) validate every TP-aware primitive at `world_size=2`.
...
* **DeepSeek-V4 weight loading** — every published V4 architecture primitive is implemented and integrated (see [ADR-014](docs/decisions/ADR-014-deepseek-v4-hybrid-attention.md)); `load_weights` walks the safetensors state_dict with FP8 dequant + MoE renames + TP dispatch. NVFP4-packed expert dequant is the last piece before real-weight smoke on 2× B200.
