...
A from-scratch LLM inference engine for newly-published architectures, with bit-parity correctness validated against the upstream reference implementation. The niche this fills, that neither vLLM nor SGLang serves: a research-paper-readable engine that ports new architectures fast and matches the reference behaviour exactly. Nine model families register today: Qwen2, Qwen3, Llama, Mistral, Gemma 3, Gemma 4, Mixtral, DeepSeek-V2, and a from-scratch implementation of every published DeepSeek-V4 primitive (hybrid CSA + HCA attention, hash-routed MoE FFN, Hyper-Connections residuals, YaRN long-context RoPE) integrated end-to-end through a runnable backbone — bit-parity validated against the upstream inference reference where applicable. Built on the production techniques those architectures depend on (continuous batching, PagedAttention with a Triton decode kernel, prefix caching, INT8 W8A16, speculative decoding, TurboQuant KV, FlashInfer FP8/NVFP4, Megatron-style tensor parallelism, prefill/decode disaggregation), every one in a textbook implementation a paper reader can also follow. Project positioning + non-goals + roadmap live in [docs/plans/roadmap-2026.md](docs/plans/roadmap-2026.md).
