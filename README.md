...
* **DeepSeek-V4 from-scratch contribution** — every published V4 primitive implemented and integrated through a runnable backbone:
   ...
   * **Backbone** (`DeepseekV4ForCausalLM`): per-layer CSA-or-HCA dispatch driven by `compress_ratios`. End-to-end prefill + decode runs with all primitives composed (CSA + HCA + Sink + Grouped Output + StateCache + hash-routed MoE + Hyper-Connections); 100+ V4-related tests cover the integration matrix. **`load_weights` still raises** — V4 weights are public ([V4-Flash](https://huggingface.co/deepseek-ai/DeepSeek-V4-Flash) 158 GB on disk, [V4-Pro](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro) 862 GB), but loading at scale requires multi-GPU tensor parallelism, which mini-infer doesn't have yet.
...
| `DeepseekV4ForCausalLM` | DeepSeek-V4-Pro (862 GB) / V4-Flash (158 GB) *(weights public; loading requires multi-GPU tensor parallelism, not yet in mini-infer)* | **Every published V4 primitive integrated end-to-end.** Attention: hybrid CSA + HCA per-layer dispatch via `compress_ratios` (ratio==4 → CSA with `LightningIndexer` + top-k; otherwise HCA), `TokenLevelCompressor` (overlap + non-overlap), `AttentionSink`, `GroupedOutputProjection`, partial RoPE, per-request `StateCache` with cache-aware prefill (aligned + unaligned) + decode. **MoE FFN** via `HashRoutedMoEFFN` with per-layer hash / score-topk routing, three score functions, shared-expert collapse. **Hyper-Connections** via `HyperConnections` (line-by-line transcription of the reference's `hc_split_sinkhorn` kernel) + `HCHeadReduction` for the LM-head collapse. **YaRN long-context RoPE** via the same `RotaryEmbedding` block V2/V3 use. Bit-parity validated against the [V4-Pro inference reference](https://huggingface.co/deepseek-ai/DeepSeek-V4-Pro/tree/main/inference) for HCA/CSA prefill and decode (cosine sim 1.0000, max abs diff < 1e-9), the MoE gate + FFN (4 routing × score_func cells), and `Block.hc_pre` / `Block.hc_post` (`hc_mult ∈ {2, 3, 4}`). `load_weights` still raises — weights are public but won't fit a single GPU; tensor parallelism is the next major piece of work. See [ADR-014](docs/decisions/ADR-014-deepseek-v4-hybrid-attention.md). |

Validated path: each loadable family has at least one CPU/MPS smoke test that loads an ungated checkpoint and produces "Paris" for `"The capital of France is"`. Mixtral 8x7B (47B params) is too large for M1 fp16 — the MoE block was bit-validated against HF's `MixtralSparseMoeBlock` on synthetic input instead. Gemma 4 31B (62 GB at bf16) and DeepSeek-V2-Lite-Chat (~31 GB at bf16) are similarly out of M1 memory budget; both validated end-to-end on B200 Modal runs. The `MLAAttention` block has bit-parity (cosine sim > 0.999) against HF's `DeepseekV2Attention` on synthetic configs. **DeepSeek-V4** runs on synthetic configs only — V4 weights ARE public (V4-Flash 158 GB, V4-Pro 862 GB) but exceed single-GPU HBM, so loading them needs tensor parallelism (not yet in mini-infer). Every published V4 primitive is bit-parity validated against the upstream inference reference, and the full backbone (CSA + HCA + StateCache + cache-aware prefill + decode + hash-routed MoE FFN + Hyper-Connections + YaRN) runs end-to-end via `scripts/demo_deepseek_v4_hybrid.py`.
...
* **Tensor parallelism** — required to load DeepSeek-V4-Flash (158 GB on disk, doesn't fit a single B200 after activations + KV) and any other future >100B-param model. mini-infer is single-device today; multi-GPU sharding (column/row-parallel linears, expert-parallel MoE, vocab-parallel embedding + LM head) is the next major infrastructure piece.
* **DeepSeek-V4 weight loading** — every published V4 architecture primitive is implemented and integrated (see [ADR-014](docs/decisions/ADR-014-deepseek-v4-hybrid-attention.md)). The remaining gates are TP (above) and the V4-specific safetensors rename rules + FP8/FP4 dequant.
