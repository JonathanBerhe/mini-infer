...
q,k = transpose(1,2)
q,k = apply_rotary_pos_emb(q,k,cos,sin) # partial RoPE: rotate first 64 of 128, pass last 64
...
idx_q,idx_k = apply_rotary_pos_emb(idx_q,idx_k,cos,sin)     # partial RoPE applied
...
- Partial RoPE: `dim=int(128*0.5)=64`; `inv_freq=1/(5e6**(arange(0,64,2)/64))`; cos/sin width 64.
...
| Partial RoPE (64/128) | reuse | `models/blocks/rope.py:309`,:144 | VERIFY NeoX/non-interleaved pairing by parity test |
...
1. RoPE pairing (NeoX half-rotation) — confirm by parity-testing rotated q/k against HF before trusting `rope.py` reuse.
